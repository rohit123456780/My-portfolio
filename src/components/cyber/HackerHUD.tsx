    ...
    <>
      <div className={`fixed top-0 left-0 right-0 z-[100] bg-background/95 border-b backdrop-blur-xl h-8 flex items-center justify-between overflow-hidden font-code text-[9px] ${mode === 'offensive' ? 'border-red-600/30' : 'border-primary/20'}`}>
          ...
          <div className="flex-1 whitespace-nowrap pl-6 flex items-center gap-8 min-w-[200px]">
            <AnimatePresence mode="wait">
              <motion.div
                ...
                exit={{ opacity: 0, x: -20 }}
                className={`flex items-center gap-3 font-bold ${mode === 'offensive' ? 'text-red-500' : 'text-primary/80'}`}
              >
                <Radio className={`w-3 h-3 ${mode === 'offensive' ? 'animate-ping' : 'animate-pulse'}`} />
                <span className="tracking-widest uppercase">{activeAlerts[currentAlert]}</span>
              </motion.div>
            </AnimatePresence>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4 pr-4 h-full">
          <button 
            ...
            className={`flex items-center gap-2 transition-colors tracking-widest uppercase border-r h-full pr-4 ${mode === 'offensive' ? 'border-red-600/20 text-red-500/60 hover:text-red-500' : 'border-primary/10 text-primary/60 hover:text-primary'}`}
          >
          ...
          <AnimatePresence>
            {!isVaultUnlocked && (
              <Link href="/vault" className={`px-6 flex items-center gap-2 tracking-widest border-l hover:bg-primary/5 transition-all group h-full ${mode === 'offensive' ? 'border-red-600/20 text-red-500/60 hover:text-red-500' : 'border-primary/10 text-primary/60 hover:text-primary'}`}>
      ...
      <nav className="fixed top-8 left-0 right-0 z-[90] p-4 flex justify-between items-center pointer-events-none">
        <Link href="/" className="pointer-events-auto">
          <div className="flex items-center gap-4 group bg-black/40 p-2 backdrop-blur-md" style={{ borderRadius: 'var(--radius)' }}>
            <div className={`w-10 h-10 border flex items-center justify-center relative ${mode === 'offensive' ? 'border-red-600/40 bg-red-600/10' : 'border-primary/40 bg-black/90'}`}>
              <Zap className={`w-5 h-5 transition-transform group-hover:scale-110 ${mode === 'offensive' ? 'text-red-500' : 'text-primary'}`} />
              <div className={`absolute -top-1 -left-1 w-2 h-2 border-t border-l ${mode === 'offensive' ? 'border-red-500' : 'border-primary'}`} />
              <div className={`absolute -bottom-1 -right-1 w-2 h-2 border-b border-r ${mode === 'offensive' ? 'border-red-500' : 'border-primary'}`} />
            </div>
            <div className={mode === 'offensive' ? 'animate-glitch' : ''}>
              ...
              <div className={`text-[7px] font-code tracking-[0.2em] uppercase ${mode === 'offensive' ? 'text-red-500/60' : 'text-primary/40'}`}>
